...
async def listar(
    ...
    try:
        ...
        # Ajuste: Sincronización con el método 'obtener_todos'
...
async def crear(
    data: dict, # Dict para evitar problemas de sesión asíncrona con el modelo
    esquema: str | None = Query(default=None)
):
    try:
        servicio = crear_servicio_registro_calificado()
...
async def actualizar(
    ...
    try:
        servicio = crear_servicio_registro_calificado()
        # Ajuste: Interpretamos el retorno booleano del repo
...
async def eliminar(
    ...
    try:
        ...
        # Primero buscamos la entidad para que el repo la maneje adecuadamente
