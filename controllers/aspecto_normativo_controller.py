...
from services.fabrica_repositorios import crear_servicio_aspecto_normativo

router = APIRouter(prefix="/aspecto_normativo", tags=["AspectoNormativo"])

@router.get("/")
async def listar(esquema: str | None = Query(default=None), limite: int | None = Query(default=None)):
    try:
        servicio = crear_servicio_aspecto_normativo()
        filas = await servicio.obtener_todos(esquema, limite)
        if not filas:
            return {"tabla": "aspecto_normativo", "total": 0, "datos": []}
        return {"tabla": "aspecto_normativo", "total": len(filas), "datos": filas}
...
async def obtener_por_id(id: int):
    try:
        ...
        if not fila:
            raise HTTPException(status_code=404, detail="Aspecto normativo no encontrado")
        return fila
