...
async def crear(
    data: dict, # Recibimos dict para mayor flexibilidad en el mapeo
    esquema: str | None = Query(default=None)
):
    try:
        servicio = crear_servicio_practica_estrategia()
        # Ajuste: El repositorio devuelve (exito, mensaje)
...
async def actualizar(
    ...
    try:
        servicio = crear_servicio_practica_estrategia()
        # Ajuste: Cambio de lógica de filas a retorno de éxito del repo
...
async def eliminar(
    ...
    try:
        ...
        # Primero buscamos el objeto para que el repositorio asíncrono lo gestione
