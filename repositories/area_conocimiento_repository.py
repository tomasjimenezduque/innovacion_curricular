...
class AreaConocimientoRepository(IRepository):
    ...
    async def guardar(self, datos: dict, esquema: str = None):
        try:
            datos.pop('id', None)
            resultado = await self.db.execute(select(func.max(AreaConocimiento.id)))
            max_id = resultado.scalar() or 0
            datos['id'] = max_id + 1

            entidad = AreaConocimiento(**datos)
            self.db.add(entidad)
            await self.db.commit()
            return True, "Área de conocimiento guardada correctamente"
        except Exception as e:
            await self.db.rollback()
            print(f"ERROR REPO AREA_CONOCIMIENTO (GUARDAR): {e}")
            return False, f"Error: {str(e)}"

    async def actualizar(self, valor_id: int, datos: dict, esquema: str = None):
        try:
            datos.pop('id', None)
            stmt = update(AreaConocimiento).where(
                AreaConocimiento.id == valor_id
            ).values(**datos)
            result = await self.db.execute(stmt)
            await self.db.commit()
    ...
    async def eliminar(self, entidad: dict, esquema: str = None):
        try:
            ...
            sql = text("DELETE FROM area_conocimiento WHERE id = :id_val")
