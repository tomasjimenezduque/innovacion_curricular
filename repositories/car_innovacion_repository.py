...
class CarInnovacionRepository(IRepository):
    ...
    async def obtener_por_id(self, valor_id: int, esquema: str = None):
        stmt = select(CarInnovacion).where(CarInnovacion.id == valor_id)
        result = await self.db.execute(stmt)
    ...
    async def guardar(self, datos: dict, esquema: str = None):
        try:
            datos.pop('id', None)
            resultado = await self.db.execute(select(func.max(CarInnovacion.id)))
            max_id = resultado.scalar() or 0
            datos['id'] = max_id + 1

            entidad = CarInnovacion(**datos)
            self.db.add(entidad)
            await self.db.commit()
            return True, "Característica de innovación guardada correctamente"
        except Exception as e:
            await self.db.rollback()
            print(f"ERROR REPO CAR_INNOVACION (GUARDAR): {e}")
            return False, f"Error: {str(e)}"

    async def actualizar(self, valor_id: int, datos: dict, esquema: str = None):
        try:
            datos.pop('id', None)
            stmt = update(CarInnovacion).where(
                CarInnovacion.id == valor_id
            ).values(**datos)
            result = await self.db.execute(stmt)
            await self.db.commit()
            if result.rowcount > 0:
                return True, "Característica actualizada correctamente"
    ...
    async def eliminar(self, entidad: dict, esquema: str = None):
        try:
            ...
            sql = text("DELETE FROM car_innovacion WHERE id = :id_val")
            await self.db.execute(sql, {"id_val": valor_id})
            await self.db.commit()
            return True, "Característica eliminada correctamente"
        except Exception as e:
            await self.db.rollback()
            return False, f"Error: {str(e)}"