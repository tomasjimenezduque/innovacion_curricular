...
class AspectoNormativoRepository(IRepository):
    ...
    async def guardar(self, datos: dict, esquema: str = None):
        try:
            datos.pop('id', None)
            resultado = await self.db.execute(select(func.max(AspectoNormativo.id)))
            max_id = resultado.scalar() or 0
            datos['id'] = max_id + 1

            entidad = AspectoNormativo(**datos)
            self.db.add(entidad)
            await self.db.commit()
            return True, "Aspecto normativo guardado correctamente"
        except Exception as e:
            await self.db.rollback()
            print(f"ERROR REPO ASPECTO_NORMATIVO (GUARDAR): {e}")
            return False, f"Error: {str(e)}"

    async def actualizar(self, valor_id: int, datos: dict, esquema: str = None):
        try:
            datos.pop('id', None)
            stmt = update(AspectoNormativo).where(
                AspectoNormativo.id == valor_id
            ).values(**datos)
            result = await self.db.execute(stmt)
            await self.db.commit()
    ...
    async def eliminar(self, entidad: dict, esquema: str = None):
        try:
            ...
            sql = text("DELETE FROM aspecto_normativo WHERE id = :id_val")
            await self.db.execute(sql, {"id_val": valor_id})
            await self.db.commit()
            return True, "Aspecto normativo eliminado correctamente"
        except Exception as e:
            await self.db.rollback()
            return False, f"Error: {str(e)}"